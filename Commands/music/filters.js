...
module.exports = {
    name: "filters",
    description: "View filters  added   to music",
    category: "Music",
    usage:`${DEFAULT_PREFIX}filters <filter name>`,
  ...
  const display_status = async (queue, embed, message) => {
    ...
    if(enabledFilters.length == 0){
      embed.setDescription(`There are currently no active filters. Use **${DEFAULT_PREFIX}f help** to view all available filters.`);
    }else{
